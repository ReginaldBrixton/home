<script setup>
import { onMounted, ref, watch } from 'vue'

definePageMeta({
  layout: 'default',
})

const theme = ref(
  typeof localStorage !== 'undefined'
    ? localStorage.getItem('theme') || 'light'
    : 'light',
)

function updateTheme() {
  if (typeof document !== 'undefined') {
    document.documentElement.classList.remove('light', 'dark')
    document.documentElement.classList.add(theme.value)
  }
}

function toggleTheme() {
  theme.value = theme.value === 'light' ? 'dark' : 'light'
  if (typeof localStorage !== 'undefined') {
    localStorage.setItem('theme', theme.value)
  }
  updateTheme()
}

onMounted(() => {
  watch(theme, updateTheme, { immediate: true })
})

defineExpose({
  toggleTheme,
})

const projects = [
  {
    id: 1,
    title: 'E-commerce Platform',
    description: 'A full-featured online store built with Vue 3 and Nuxt.',
    technologies: ['Vue', 'Nuxt', 'TailwindCSS'],
    image: 'https://images.unsplash.com/photo-1557821552-17105176677c?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    link: 'https://github.com',
  },
  {
    id: 2,
    title: 'Task Management App',
    description: 'Real-time collaborative task management application.',
    technologies: ['Vue', 'Node.js', 'MongoDB'],
    image: 'https://images.unsplash.com/photo-1454165804606-c3d57bc86b40?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    link: 'https://github.com',
  },
  {
    id: 3,
    title: 'Weather Dashboard',
    description: 'Interactive weather visualization using modern APIs.',
    technologies: ['Vue', 'REST API', 'Chart.js'],
    image: 'https://images.unsplash.com/photo-1592210454359-9043f067919b?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    link: 'https://github.com',
  },
]

const skills = [
  { name: 'Vue.js', level: 90 },
  { name: 'Node.js', level: 85 },
  { name: 'JavaScript', level: 95 },
  { name: 'HTML/CSS', level: 90 },
  { name: 'MongoDB', level: 80 },
  { name: 'AWS', level: 75 },
]

const experiences = [
  {
    company: 'Tech Corp',
    position: 'Senior Frontend Developer',
    period: '2020 - Present',
    description: 'Leading frontend development team, implementing modern web solutions.',
  },
  {
    company: 'Digital Solutions Inc',
    position: 'Full Stack Developer',
    period: '2018 - 2020',
    description: 'Developed and maintained multiple web applications using Vue.js and Node.js.',
  },
]
</script>

<template>
  <div class="space-y-16 dark:bg-gray-900">
    <!-- Hero Section -->
    <section class="relative h-screen flex items-center justify-center text-center">
      <div class="absolute inset-0 -z-10">
        <img
          src="https://images.unsplash.com/photo-1517134191118-9d595e4c8c2b?ixlib=rb-4.0.3"
          class="w-full h-full object-cover opacity-10"
          alt="Background"
        >
      </div>
      <div class="space-y-6 px-4">
        <img
          src="https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?ixlib=rb-4.0.3"
          class="w-32 h-32 rounded-full mx-auto shadow-lg"
          alt="Profile"
        >
        <h1 class="text-6xl font-bold text-gray-900 dark:text-white">
          John Doe
        </h1>
        <p class="text-2xl text-gray-600 dark:text-gray-300">
          Full Stack Developer
        </p>
        <div class="flex justify-center gap-4">
          <a
            href="#projects"
            class="px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors"
          >
            View Projects
          </a>
          <a
            href="#contact"
            class="px-6 py-3 bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-white rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors"
          >
            Contact Me
          </a>
          <button
            @click="toggleTheme"
            class="p-3 bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-white rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors"
          >
            {{ theme === 'light' ? '🌙' : '☀️' }}
          </button>
        </div>
      </div>
    </section>

    <!-- About Section -->
    <section id="about" class="container mx-auto px-4 space-y-8">
      <h2 class="text-4xl font-bold text-center text-gray-900 dark:text-white">
        About Me
      </h2>
      <div class="max-w-3xl mx-auto text-lg text-gray-600 dark:text-gray-300 space-y-4">
        <p>
          I'm a passionate developer with 5 years of experience building modern web applications.
          Specialized in Vue.js, Node.js, and cloud technologies.
        </p>
        <p>
          My goal is to create efficient, scalable, and user-friendly applications that solve real-world problems.
        </p>
      </div>

      <!-- Skills -->
      <div class="mt-12 grid grid-cols-1 md:grid-cols-2 gap-8">
        <div
          v-for="skill in skills"
          :key="skill.name"
          class="space-y-2"
        >
          <div class="flex justify-between">
            <span class="font-medium dark:text-white">{{ skill.name }}</span>
            <span class="text-gray-500 dark:text-gray-400">{{ skill.level }}%</span>
          </div>
          <div class="h-2 bg-gray-200 dark:bg-gray-700 rounded-full">
            <div
              class="h-full bg-blue-600 rounded-full"
              :style="`width: ${skill.level}%`"
            />
          </div>
        </div>
      </div>
    </section>

    <!-- Projects Section -->
    <section id="projects" class="container mx-auto px-4 space-y-8">
      <h2 class="text-4xl font-bold text-center text-gray-900 dark:text-white">
        Featured Projects
      </h2>
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        <BaseCard
          v-for="project in projects"
          :key="project.id"
          class="group hover:transform hover:scale-105 transition-transform duration-300"
        >
          <img
            :src="project.image"
            :alt="project.title"
            class="w-full h-48 object-cover rounded-t-lg"
          >
          <div class="p-6 space-y-4">
            <h3 class="text-xl font-semibold text-gray-800 dark:text-white">
              {{ project.title }}
            </h3>
            <p class="text-gray-600 dark:text-gray-300">
              {{ project.description }}
            </p>
            <div class="flex flex-wrap gap-2">
              <span
                v-for="tech in project.technologies"
                :key="tech"
                class="px-3 py-1 bg-gray-100 dark:bg-gray-700 rounded-full text-sm text-gray-700 dark:text-gray-300"
              >
                {{ tech }}
              </span>
            </div>
            <a
              :href="project.link"
              target="_blank"
              class="inline-block px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors"
            >
              View Project
            </a>
          </div>
        </BaseCard>
      </div>
    </section>

    <!-- Experience Section -->
    <section class="container mx-auto px-4 space-y-8">
      <h2 class="text-4xl font-bold text-center text-gray-900 dark:text-white">
        Experience
      </h2>
      <div class="max-w-3xl mx-auto space-y-8">
        <div
          v-for="exp in experiences"
          :key="exp.company"
          class="border-l-4 border-blue-600 pl-4 space-y-2"
        >
          <h3 class="text-xl font-semibold text-gray-800 dark:text-white">
            {{ exp.position }}
          </h3>
          <p class="text-blue-600">
            {{ exp.company }}
          </p>
          <p class="text-gray-500 dark:text-gray-400">
            {{ exp.period }}
          </p>
          <p class="text-gray-600 dark:text-gray-300">
            {{ exp.description }}
          </p>
        </div>
      </div>
    </section>

    <!-- Contact Section -->
    <section id="contact" class="container mx-auto px-4 space-y-8">
      <h2 class="text-4xl font-bold text-center text-gray-900 dark:text-white">
        Get In Touch
      </h2>
      <div class="flex justify-center gap-8">
        <a
          href="https://github.com"
          target="_blank"
          class="flex items-center gap-2 text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white"
        >
          <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
            <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
          </svg>
          GitHub
        </a>
        <a
          href="https://linkedin.com"
          target="_blank"
          class="flex items-center gap-2 text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white"
        >
          <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
            <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" />
          </svg>
          LinkedIn
        </a>
        <a
          href="mailto:john@example.com"
          class="flex items-center gap-2 text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white"
        >
          <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
          </svg>
          Email
        </a>
      </div>
    </section>
  </div>
</template>
